...
router.get('/clients/:id/bank-recommendations', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const result = rankWithPatternMatching(input, policies);
    ...
    res.json({ scenarioUsed: input.scenario, patterns: result.patterns, clusterBrandCodes: result.clusterBrandCodes, top3: all.slice(0, 3), all, explanations: explanationByBrand });
  } catch (error) {
