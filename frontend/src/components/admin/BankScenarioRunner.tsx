...
function defaultScenario() {
  return {
    ...
    debts: [{ id: 'd1', type: 'CREDIT_CARD', source: 'STANDALONE', lender: 'Visa', creditLimit: 15000 }],
...
function ScenarioSummary({ sc }: { sc: any }) {
  if (!sc) return null;
  return (
    <p className="tnum rounded-lg bg-white/5 px-3 py-2 text-xs text-muted">
      Scenario used: {String(sc.purpose).replace('_', ' ')} · target loan {money(sc.targetLoanAmount)} · property {money(sc.targetPropertyValue)} · {sc.termYears}yr · {(sc.interestRate * 100).toFixed(2)}% {sc.repaymentType}
    </p>
  );
}

function RecCard({ r, rank }: { r: any; rank: number }) {
  return (
    <div className="glass-2 rounded-2xl p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          {rank < 3 && <span className="text-lg">{medal[rank]}</span>}
...
export function BankScenarioRunner() {
  ...
  // --- Client-driven mode (primary) ---
  const [clients, setClients] = useState<Array<{ id: string; name: string; email: string }>>([]);
  const [clientId, setClientId] = useState('');
  const [recs, setRecs] = useState<any[]>([]);
  const [scenarioUsed, setScenarioUsed] = useState<any>(null);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const list = (res.data.clients || []).map((c: any) => ({ id: c.id, name: c.name, email: c.email }));
        setClients(list);
      } catch { /* ignore */ }
    })();
  }, []);

  const runForClient = async () => {
    if (!clientId) { toast('Select a client first', { accent: 'gold' }); return; }
    setRunning(true);
    try {
      const res = await api.get(`/admin/clients/${clientId}/bank-recommendations`);
      setRecs(res.data.all || []);
      setScenarioUsed(res.data.scenarioUsed || null);
    } catch (e: any) {
      toast(e?.response?.data?.error || 'Could not extract this client’s data', { accent: 'crimson' });
      setRecs([]); setScenarioUsed(null);
    } finally {
      setRunning(false);
    }
  };
  ...
  const runManual = async (payload = s) => {
    setManualRunning(true);
    try {
      const res = await api.post('/bank-policies/rank', payload);
      setManualRecs(res.data.recommendations || []);
    } catch {
      toast('Could not rank banks', { accent: 'crimson' });
    } finally {
  ...
  const brands = manualRecs.map((r) => r.brandCode);
  ...
  return (
    <div className="space-y-5">
      {/* Client-driven: pick a client, auto-extract, run */}
      <div className="glass-2 rounded-2xl p-5">
        <h3 className="mb-1 font-display text-base font-semibold text-primary">Find the best banks for a client</h3>
        <p className="mb-3 text-sm text-secondary">Pick a client — their income, expenses, properties, debts and latest scenario are pulled automatically. No re-entry needed.</p>
        <div className="flex flex-wrap items-end gap-3">
          <div className="min-w-[260px] flex-1">
            <Select
              label="Client"
              options={[{ value: '', label: clients.length ? 'Select a client…' : 'Loading clients…' }, ...clients.map((c) => ({ value: c.id, label: `${c.name} — ${c.email}` }))]}
              value={clientId}
              onChange={(e) => setClientId(e.target.value)}
            />
          </div>
          <button type="button" onClick={runForClient} disabled={running || !clientId} className="ripple-btn rounded-xl bg-gradient-to-br from-brand to-brand-dark px-5 py-2.5 text-sm font-semibold text-on-accent shadow-lg shadow-brand/30 hover:brightness-110 disabled:opacity-50">
            {running ? 'Analysing…' : 'Run & suggest top 3'}
          </button>
        </div>
        {scenarioUsed && <div className="mt-3"><ScenarioSummary sc={scenarioUsed} /></div>}
      </div>

      {/* Recommendations (client-driven) */}
      {recs.length > 0 && (
        <div className="space-y-3">
          <h3 className="font-display text-base font-semibold text-primary">Top 3 banks to approach</h3>
          {recs.slice(0, 3).map((r, i) => <RecCard key={r.brandCode} r={r} rank={i} />)}
          {recs.length > 3 && (
            <details>
              <summary className="cursor-pointer text-xs font-semibold text-brand">Show all {recs.length} lenders</summary>
              <div className="mt-2 space-y-3">{recs.slice(3).map((r) => <RecCard key={r.brandCode} r={r} rank={9} />)}</div>
            </details>
          )}
        </div>
      )}

      {/* Manual builder (optional fallback) */}
      <details className="glass-2 rounded-2xl p-5">
        <summary className="cursor-pointer font-display text-base font-semibold text-primary">Or build a manual scenario</summary>
        <div className="mt-4 space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                <p className="text-xs font-semibold uppercase tracking-wide text-muted">Properties</p>
          ...
          <div className="flex justify-end">
            <button type="button" onClick={() => runManual()} disabled={manualRunning} className="rounded-xl bg-gradient-to-br from-brand to-brand-dark px-5 py-2.5 text-sm font-semibold text-on-accent shadow-lg shadow-brand/30 hover:brightness-110 disabled:opacity-50">
              {manualRunning ? 'Ranking…' : 'Rank banks'}
            </button>
          </div>

          {manualRecs.length > 0 && (
            <div className="space-y-3">
              {manualRecs.map((r, i) => <RecCard key={r.brandCode} r={r} rank={i} />)}
              {s.properties.length > 0 && (
                <div className="overflow-x-auto rounded-2xl border border-white/10 p-4">
                  ...
                  <p className="mb-3 text-xs text-muted">Toggle a property off for a bank; that bank re-ranks. ✓ = considered after the bank&apos;s selection cap.</p>
                      ...
                      {s.properties.map((p: any, i: number) => (
                        <tr key={p.id} className="border-t border-white/8">
                          ...
                          {brands.map((b) => {
                            ...
                            const considered = manualRecs.find((r) => r.brandCode === b)?.calcResult.propertiesConsidered.includes(p.id);
                            return (
